...
class EMRegistrationProjection(object):
    """
    ...
    def __init__(self, X_raw, X, Y, Z=None, adjW=None, K=1, sigma2=None, max_iterations=None, tolerance=None, w=None, *args, **kwargs):
    ...
    def register(self, callback=lambda **kwargs: None):
        ...
        while self.iteration < self.max_iterations and self.diff > self.tolerance:
            ...
            if callable(callback):# and self.iteration  % 100 == 0:
                kwargs = {'iteration': self.iteration,
                          'error': self.q, 'X': self.X_raw, 'Y': self.Y, 'TY': self.TY, 'Z': self.Z,
                          'vis': self.vis}
                callback(**kwargs)
                #print(self.epsilon)
    ...
    def expectation(self):
        ...
        for k in range(self.K):
            P_1[:, :, k] = np.sum((self.X[None, :, :self.D-1]
                                   - self.TY[k][:, None, :self.D-1]) ** 2, axis=2)
