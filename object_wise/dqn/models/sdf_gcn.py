...
class SDFGCNQNetV4(nn.Module):
    ...
    def forward(self, sdfs, nsdf):
        ...
        boundary[idx_sdf] = (sdfs[idx_sdf].abs() < eps).type(torch.float)
        ...
        sdfs_concat = torch.cat([sdfs.unsqueeze(2), 
                                boundary.unsqueeze(2), 
                                block_flags.unsqueeze(2)], 2)   # bs x 2nb x 3 x h x w
        x_conv1 = self.gcn1(sdfs_concat, adj_matrix)        # bs x 2nb x c x h x w
        x_conv2 = self.gcn2(x_conv1, adj_matrix)            # bs x 2nb x cout x h x w
        x_average = torch.mean(x_conv2, dim=(3, 4))         # bs x 2nb x cout
...
class SDFGCNQNetV5(nn.Module):
    def __init__(self, num_blocks, n_actions=8, n_hidden=16, normalize=False):
        super(SDFGCNQNetV5, self).__init__()
        self.n_actions = n_actions
        self.num_blocks = num_blocks
        self.normalize = normalize
    ...
    def generate_adj(self):
        ...
        for nb in range(1, self.num_blocks + 1):
            ...
            adj_matrix[nb - 1, self.num_blocks:self.num_blocks + nb, :nb] = torch.eye(nb)
            adj_matrix[nb - 1, :nb, self.num_blocks:self.num_blocks + nb] = torch.eye(nb)
            adj_matrix[nb - 1, self.num_blocks:self.num_blocks + nb, self.num_blocks:self.num_blocks + nb] = torch.ones([nb, nb])
            if self.normalize:
                diag = [1/np.sqrt(nb+1)] * nb
                diag += [0] * (self.num_blocks - nb)
                diag += [1/np.sqrt(nb+1)] * nb
...
class SDFGCNQNetV6(nn.Module):
    def __init__(self, num_blocks, n_actions=8, n_hidden=16, normalize=False):
        super(SDFGCNQNetV6, self).__init__()
        self.n_actions = n_actions
        self.num_blocks = num_blocks
    ...
    def generate_adj(self, nsdf):
        ...
        for ns in nsdf:
            ...
            _adj[:ns, NB:NB+ns] = torch.eye(int(ns))
            #_adj[NB:NB+ns, :ns] = torch.eye(int(ns))
            adj.append(_adj.unsqueeze(0))
        return torch.cat(adj, 0).to(device)
...
class SDFGCNQNetV7(nn.Module):
    def __init__(self, num_blocks, n_actions=8, n_hidden=16, normalize=False):
        super(SDFGCNQNetV7, self).__init__()
        self.n_actions = n_actions
        self.num_blocks = num_blocks
...
class SDFGCNQNetV8(nn.Module):
    def __init__(self, num_blocks, n_actions=8, n_hidden=16, normalize=False):
        super(SDFGCNQNetV8, self).__init__()
        self.n_actions = n_actions
        self.num_blocks = num_blocks
    ...
    def generate_adj(self, nsdf):
        ...
        for ns in nsdf:
            ...
            if int(ns) > 0:
                _adj[:ns, :ns] = 1/np.sqrt(int(ns)) * (torch.ones(ns, ns) - torch.eye(int(ns))) 
            _adj[:ns, NB:NB+ns] = torch.eye(int(ns))
            _adj[NB:NB+ns, NB:NB+ns] = torch.eye(int(ns))
            adj.append(_adj.unsqueeze(0))
        return torch.cat(adj, 0).to(device)
