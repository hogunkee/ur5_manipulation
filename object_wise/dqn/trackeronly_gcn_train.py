...
def learning(env, 
    ...
    print('='*30)
    qnet = QNet(max_blocks, env.num_blocks, n_actions, normalize=graph_normalize).to(device)
    if pretrain:
    ...
    qnet_target = QNet(max_blocks, env.num_blocks, n_actions, normalize=graph_normalize).to(device)
