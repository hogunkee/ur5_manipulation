...
def learning(env, 
    ...
    print('='*30)
    qnet = QNet(max_blocks, n_actions, n_hidden=64, normalize=graph_normalize, resize=sdf_module.resize).to(device)
    if pretrain:
    ...
    qnet_target = QNet(max_blocks, n_actions, n_hidden=64, normalize=graph_normalize).to(device)
