    ...
    for ne in range(total_episodes):
        ...
        for t_step in range(env.max_steps):
            ...
            if per:
                ...
                if her and not done:
                    if round_sdf:
                        sdf_ag = sdf_module.make_round_sdf(sdf_ns_align)
                    else:
                        sdf_ag = sdf_ns_align
...
if __name__=='__main__':
    ...
    parser.add_argument("--reward", default="maskpenalty", type=str)
    # learning params #
    ...
    env = objectwise_env(env, num_blocks=num_blocks, mov_dist=mov_dist, max_steps=max_steps, \
            threshold=threshold, conti=False, detection=True, reward_type='linear')
