    ...
    for ne in range(int(args.total_episodes)):
        ...
        while not check_env_ready:
            ...
            if True:
                ...
                fnum = len([f for f in os.listdir('test/') if f.startswith('d')])
                im.save('test/d%d.png'%fnum)
                print('test/d%d.png'%fnum, 'saved.')
            obs.append([state_img[0], goal_img[0]])
        ...
        for t_step in range(_env.max_steps):
            ...
            if len(obs)>=10:
                for st, g in obs:
                    ...
                    im.save('test/t%d.png'%fnum)
                    print('test/t%d.png'%fnum, 'saved.')
                exit()
            count_steps += 1
            ep_len += 1
            ...
            if args.oracle:
                ...
                sdf_raw = sdf_module.align_sdf(matching, sdf_raw, np.zeros([_env.num_blocks, *sdf_raw.shape[1:]]))
            obs.append([next_state_img[0], _[0]])
