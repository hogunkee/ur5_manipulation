...
def get_action(env, fc_qnet, fc_qnet2, fc_qnet3, state, epsilon, pre_action=None, with_q=False, sample='sum', output='addR'):
    if np.random.random() < epsilon:
        ...
        if with_q:
            ...
            state_goal_q1 = torch.cat((state_im, goal_im, q1_value), 1)
            q2_value = fc_qnet2(state_goal_q1, True)
            state_goal_q2 = torch.cat((state_im, goal_im, q2_value), 1)
            q3_value = fc_qnet3(state_goal_q2, True)

            q1_raw = q1_value[0].detach().cpu().numpy()  # q_raw: 8 x 96 x 96
            q2_raw = q2_value[0].detach().cpu().numpy()  # q_raw: 8 x 96 x 96
            q3_raw = q3_value[0].detach().cpu().numpy()  # q_raw: 8 x 96 x 96

            q_raw = np.concatenate([q1_raw, q2_raw, q3_raw]).reshape(env.num_blocks, 8, 96, 96)
        ...
        if output=='':
            ...
            q[:, crop_min:crop_max, crop_min:crop_max] += q3_raw[:, crop_min:crop_max, crop_min:crop_max]
        elif output=='addQ':
            for o in range(env.num_blocks):
                q[:, crop_min:crop_max, crop_min:crop_max] += q_raw[o, :, crop_min:crop_max, crop_min:crop_max]
...
def evaluate(env, n_blocks=3, in_channel=[6,14,14], model1='', model2='', model3='', num_trials=10, visualize_q=False, sampling='uniform', output='addQ'):
    ...
    FCQ2 = FC_QNet(8, in_channel[1]).type(dtype)
    print('Loading trained PCQN model: {}'.format(model2))
    FCQ2.load_state_dict(torch.load(model2))
    FCQ3 = FC_QNet(8, in_channel[2]).type(dtype)
    print('Loading trained PCQN3B model: {}'.format(model3))
    FCQ3.load_state_dict(torch.load(model3))
...
def learning(env, 
    ...
    if continue_learning:
        FCQ3.load_state_dict(torch.load(model3))
    FCQ3_target = FC_QNet(8, in_channel[1]).type(dtype)
    FCQ3_target.load_state_dict(FCQ3.state_dict())

    optimizer = torch.optim.SGD(FCQ3.parameters(), lr=learning_rate, momentum=0.9, weight_decay=2e-5)
    ...
    model2_parameters = filter(lambda p: p.requires_grad, FCQ2.parameters())
    model3_parameters = filter(lambda p: p.requires_grad, FCQ3.parameters())
    params = sum([np.prod(p.size()) for p in model1_parameters])
    params += sum([np.prod(p.size()) for p in model2_parameters])
    params += sum([np.prod(p.size()) for p in model3_parameters])
    print("# of params: %d"%params)


    if double:
        calculate_cascade_loss = calculate_cascade_loss_double_cascade_3blocks
    else:
        calculate_cascade_loss = calculate_cascade_loss_cascade_3blocks
    ...
    if continue_learning:
        ...
        log_success_b3 = numpy_log[9].tolist()
    else:
        log_returns = []
        log_loss = []
        log_eplen = []
    ...
    while t_step < total_steps:
        ...
        if done:
            ...
            if ne % log_freq == 0:
                ...
                log_mean_success_b3 = smoothing_log(log_success_b3, log_freq)
                ...
                axes[0][1].plot(log_mean_success_b2, color='red') #2
                axes[0][2].plot(log_mean_success_b3, color='red') #3
                ...
                if log_mean_success[-1] > max_success:
                    ...
                    torch.save(FCQ3.state_dict(), 'results/models/%s.pth' % savename)
...
if __name__=='__main__':
    ...
    parser.add_argument("--model1_path", default="0627_1554", type=str)
    parser.add_argument("--model2_path", default="0630_1616", type=str)
    parser.add_argument("--model3_path", default="####_####", type=str)
    ...
    model3_path = os.path.join("results/models/PCQN3B_%s.pth" % args.model3_path)
    ...
    savename = "PCQN3B_%s" % (now.strftime("%m%d_%H%M"))
    if not evaluation:
