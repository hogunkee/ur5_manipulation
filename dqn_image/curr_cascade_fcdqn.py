...
from target_pushpixel_env import *
from utils import *
...
def get_action(env, fc_qnet, cqn, state, epsilon, pre_action=None, with_q=False):
    if np.random.random() < epsilon:
        ...
        if with_q:
            ...
            q1_value = fc_qnet(state_goal)
            state_goal_q = torch.cat((state_im, goal_im, q1_value), 1)
            q2_value = cqn(state_goal_q)
...
def evaluate(env, n_blocks=3, in_channel=[6,14], model1='', model2='', num_trials=10, visualize_q=False, sampling='uniform', targets=[]):
    FCQ = FC_QNet(8, in_channel[0]).type(dtype)
    print('Loading trained CU_FCDQN model: {}'.format(model1))
    FCQ.load_state_dict(torch.load(model1))
    CQN = FC_QNet(8, in_channel[1]).type(dtype)
    print('Loading trained CU_PCQN model: {}'.format(model2))
    ...
    while ne < num_trials:
        action, q_map, q_raw = get_action(env, FCQ, CQN, state, epsilon=0.0, pre_action=pre_action, with_q=True)
        if visualize_q:
            s0 = deepcopy(state[0]).transpose([1, 2, 0])
            if env.goal_type == 'pixel':
                s1 = np.zeros([env.env.camera_height, env.env.camera_width, 3])
...
def learning(env, 
    ...
    if double:
        calculate_cascade_loss = calculate_loss_double_curr_pcqn
    else:
    ...
    while t_step < total_steps:
        action, q_map, _ = get_action(env, FCQ, CQN, state, epsilon=epsilon, pre_action=pre_action, with_q=True)
...
if __name__=='__main__':
    ...
    parser.add_argument("--model1_path", default="0708_0818", type=str)
    ...
    model1_path = os.path.join("results/models/CU_FCDQN_%s.pth"%args.model1_path)
    model2_path = os.path.join("results/models/CU_PCQN_%s.pth"%args.model2_path)
    ...
    savename = "CU_PCQN_%s" % (now.strftime("%m%d_%H%M"))
    if not evaluation:
    ...
    env = target_pushpixel_env(env, num_blocks=num_blocks, mov_dist=mov_dist,max_steps=max_steps,\
