...
def learning(env, 
    ...
    replay_buffer = ReplayBuffer([3, env.env.camera_height, env.env.camera_width], 1, \
            save_goal=(env.task==1), max_size=int(buff_size))
    ...
    def get_action(q_net, state, epsilon):
        if np.random.random() < epsilon:
            ...
            if env.task==0:
                ...
                state_gp = torch.tensor([state[1]]).type(dtype)
                q_value = q_net(state_im, state_gp)
                action = np.argmax(q_value.detach().cpu().numpy())
            else:
                state_im = torch.tensor([state[0]]).type(dtype)
                goal_im = torch.tensor([state[1]]).type(dtype)
                state_gp = torch.tensor([state[2]]).type(dtype)
                state_goal = torch.cat((state_im, goal_im), 1)
                print(state_goal)
                q_value = q_net(state_goal, state_gp)
                action = np.argmax(q_value.detach().cpu().numpy())
        return action
    ...
    def calculate_loss_origin(minibatch, gamma=0.99):
        ...
        state_gp = minibatch[1]
        next_state_im = minibatch[2]
        next_state_gp = minibatch[3]
        if env.task==0:
            state = state_im
        ...
        next_q = Q_target(next_state, next_state_gp)
    ...
    def calculate_loss_double(minibatch, gamma=0.99):
        ...
        next_q = Q(next_state, next_state_gp).detach()
        _, a_prime = next_q.max(1)

        q_target_next = Q_target(next_state, next_state_gp).detach()
