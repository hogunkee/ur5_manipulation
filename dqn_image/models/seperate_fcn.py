...
class FC_QNet(nn.Module):
    def __init__(self, n_actions, in_channel, n_blocks):
        ...
        self.fully_conv = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout2d(),
                nn.Conv2d(512, n_blocks, kernel_size=1),
                )

        # self.upscore = nn.ConvTranspose2d(1, 1, 16, stride=8, bias=False)
        self.upscore = nn.Sequential(
            nn.ConvTranspose2d(n_blocks, n_blocks, 3, stride=2, bias=False, padding=1, output_padding=1),
            nn.ConvTranspose2d(n_blocks, n_blocks, 3, stride=2, bias=False, padding=1, output_padding=1),
            nn.ConvTranspose2d(n_blocks, n_blocks, 3, stride=2, bias=False, padding=1, output_padding=1),
        )
    ...
    def forward(self, x, is_volatile=False, rotation=-1, debug=False):
        ...
        return torch.cat(output_prob, 2) # bs x nb x 8 x h x w
...
class FC_QNet_half(nn.Module):
    def __init__(self, n_actions, in_channel, n_blocks):
        ...
        self.fully_conv = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout2d(),
                nn.Conv2d(256, n_blocks, kernel_size=1),
                )
...
class FC_Q2Net(nn.Module):
    def __init__(self, n_actions, in_channel, n_blocks):
        super(FC_Q2Net, self).__init__()
        self.n_actions = n_actions
        self.n_blocks = n_blocks
        ...
        self.fully_conv = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout2d(),
                nn.Conv2d(512, 2*n_blocks, kernel_size=1),
                )
...
class FC_Q2Net_half(nn.Module):
    def __init__(self, n_actions, in_channel, n_blocks):
        super(FC_Q2Net_half, self).__init__()
        self.n_actions = n_actions
        self.n_blocks = n_blocks
        ...
        self.fully_conv = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout2d(),
                nn.Conv2d(256, 2*n_blocks, kernel_size=1),
                )
