...
class FC_QNet(nn.Module):
    def __init__(self, n_actions, task):
        ...
        self.cnn = nn.Sequential(
                ...
                nn.ReLU(),
                # nn.MaxPool2d(2, stride=2, ceil_mode=True),
                nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=3, dilation=2),
                nn.BatchNorm2d(256),
                nn.ReLU(),
    ...
    def forward(self, x, is_volatile=False, rotation=-1):
        ...
        if is_volatile:
            for r_idx in range(self.n_actions):
                ...
                flow_grid_before = F.affine_grid(Variable(affine_mat_before, requires_grad=False).cuda(), x_pad.size())
                ...
                flow_grid_after = F.affine_grid(Variable(affine_mat_after, requires_grad=False).cuda(), h.size())

                h_after = F.grid_sample(h, flow_grid_after, mode='nearest')
                h_after = h_after[:, :, 22:22 + x.size()[2], 22:22 + x.size()[3]].contiguous()
