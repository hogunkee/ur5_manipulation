...
def learning(env, 
    ...
    while t_step < total_steps:
        ...
        if per:
            ...
            q_value = FCQ(state_goal)[0].data
            next_q_target = FCQ_target(next_state_goal)[0].data
            if double:
                next_q_value = FCQ(next_state_goal)[0].data
            ...
            for o in range(n_blocks):
                if done:
                    ...
                    if double:
                        ...
                        target_val = rewards[o] + gamma * torch.max(next_q_target[o])
                    ...
                    for o in range(n_blocks):
                        if done_re:
                            ...
                            if double:
                                ...
                                target_val = rewards_re[o] + gamma * torch.max(next_q_target[o])
                        old_val = q_value[o, action[2], action[0], action[1]]
                        error += abs(old_val - target_val).data.detach().cpu().numpy()
...
if __name__=='__main__':
    ...
    half = True #args.half
    if half:
        from models.cqn import FC_QNet_half as FC_QNet
    else:
        from models.cqn import FC_QNet

    # evaluate configuration #
    evaluation = args.evaluate
    model_path = os.path.join("results/models/CQN2_%s.pth"%sargs.model_path)
    ...
    savename = "CQN2_%s" % (now.strftime("%m%d_%H%M"))
    if not evaluation:
