...
def evaluate(env, n_actions=8, model_path='', num_trials=10, visualize_q=False):
    ...
    while ne < num_trials:
        ...
        if done:
            ...
            if target in log_success_per_obj:
                log_success_per_obj[target].append(int(info['success']))
            else:
                log_success_per_obj[target] = [int(info['success'])]
            if info['reached_obj'] != -1:
                num_reaches += 1
            ...
            print("Reach the goal: {}/{}".format(num_reaches, ne))
    ...
    print("="*80)
    for t_obj in range(target+1):
        print("Object {}: {}/{}".format(t_obj, np.sum(log_success_per_obj[t_obj]), len(log_success_per_obj[t_obj])))
    print("=" * 80)
    print("Reach the goal: {}/{}".format(num_reaches, num_trials))
...
def learning(env, 
    ...
    def sample_her_transitions(info, num_samples=1):
        archived_obj = info['reached_obj']
        goal_image = env.object_images[archived_obj]
        goal_image = np.transpose(goal_image, [2, 0, 1])
        reward = 1.0
        return reward, goal_image, True
    ...
    axes[1][1].set_title('Num Collisions')
    ...
    while t_step < total_steps:
        ...
        ## HER ##
        if her and info['reached_obj']!=-1 and env.task==1:
            reward_re, goal_image, done_re  = sample_her_transitions(info, num_samples=1)
            if per:
                state_im = torch.tensor([state[0]]).type(dtype)
        ...
        if done:
            ...
            if ne % log_freq == 0:
                ...
                axes[1][1].plot(log_collisions, color='#ff33cc', linewidth=0.5)
                axes[2][1].plot(log_epsilon, color='black', linewidth=0.5)
                ...
                axes[1][1].plot(log_mean_collisions, color='#663399')
...
if __name__=='__main__':
    ...
    parser.add_argument("--num_use", default=4, type=int)
    parser.add_argument("--num_select", default=4, type=int)
    parser.add_argument("--dist", default=0.10, type=float)
    ...
    parser.add_argument("--num_trials", default=160, type=int)
    ...
    env = UR5Env(render=render, camera_height=camera_height, camera_width=camera_width, \
            control_freq=5, data_format='NCHW', xml_ver=task+1)
    env = targetpush_env(env, num_use=num_use, num_select=num_select, mov_dist=mov_dist, max_steps=max_steps, task=task)
    ...
    if evaluation:
        ...
        learning(env=env, n_actions=8, learning_rate=learning_rate, batch_size=batch_size, \
                 buff_size=buff_size, total_steps=total_steps, learn_start=learn_start, \
                 update_freq=update_freq, log_freq=log_freq, double=double, her=her, per=per, \
                 visualize_q=visualize_q)
