...
def get_action(env, cqn, state, epsilon, pre_action=None, with_q=False):
...
def evaluate(env, n_blocks=3, in_channel=14, model='', num_trials=10, visualize_q=False, sampling='uniform', targets=[]):
    CQN = FC_QNet(8, in_channel).type(dtype)
    print('Loading trained CU_REQN model: {}'.format(model))
...
def learning(env, 
    ...
    model_parameters = filter(lambda p: p.requires_grad, CQN.parameters())
...
if __name__=='__main__':
    ...
    model_path = os.path.join("results/models/CU_REQN_%s.pth"%args.model_path)
    ...
    savename = "CU_REQN_%s" % (now.strftime("%m%d_%H%M"))
    if not evaluation:
    ...
    if evaluation:
        evaluate(env=env, n_blocks=num_blocks, in_channel=in_channel, model=model_path, \
