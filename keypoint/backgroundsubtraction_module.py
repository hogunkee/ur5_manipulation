...
class BackgroundSubtraction():
    ...
    def fitting_model(self):
        ...
        pad = self.pad 
        data_dir = os.path.join(FILE_PATH, '../object_wise/dqn/test_scenes')

        for i in range(100, 185):
            img_s = np.array(Image.open(os.path.join(data_dir, 'state/%d.png'%i)))
            img_s[:40, 120:240] = [209, 238, 255]
            self.model.apply(img_s)

            img_g = np.array(Image.open(os.path.join(data_dir, 'goal/%d.png'%i)))
            img_g[:40, 120:240] = [209, 238, 255]
            self.model.apply(img_g)

    def get_masks(self, image, n_cluster=3):
        image[:40, 120:240] = [209, 238, 255]
