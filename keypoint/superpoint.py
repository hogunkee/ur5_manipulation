...
class SuperPointFrontend(object):
  ...
  def run_rotate(self, img):
    ...
    for theta in range(4):
        _inp = img.copy()
        _inp = rotate(_inp, angle=90*theta, reshape=True)
        _inp = (_inp.reshape(1, 1, H, W))
        inp.append(_inp)
    inp = np.concatenate(inp, axis=0)
    inp = torch.from_numpy(inp)
    inp = torch.autograd.Variable(inp).view(4, 1, H, W)
    if self.cuda:
      inp = inp.cuda()
    # Forward pass of network.
    outs = self.net.forward(inp)
    ...
    if pts.shape[1] == 0:
      ...
      for i in range(4):
          ...
          _pts = pts[:2, :].copy()
          _pts_rotated = np.dot(R, (_pts - np.array([[240], [240]]))) + np.array([[240], [240]])

          samp_pts = torch.from_numpy(_pts_rotated)
          samp_pts[0, :] = (samp_pts[0, :] / (float(W)/2.)) - 1.
          samp_pts[1, :] = (samp_pts[1, :] / (float(H)/2.)) - 1.
          samp_pts = samp_pts.transpose(0, 1).contiguous()
          samp_pts = samp_pts.view(1, 1, -1, 2)
          ...
          if self.cuda:
            samp_pts = samp_pts.cuda()
          _desc = torch.nn.functional.grid_sample(coarse_desc[i:i+1], samp_pts)
          _desc = _desc.data.cpu().numpy().reshape(D, -1)
          _desc /= np.linalg.norm(_desc, axis=0)[np.newaxis, :]
          desc.append(_desc)
      desc = np.array(desc)
    return pts, desc, heatmap
...
class PointTracker(object):
  ...
  row_m
  fe= [track_id_m, avg_desc_score_m, point_id_0_m, ..., point_id_L-1_m].
  """
